  ...
  return (
    <section id="product" className="relative h-screen flex items-center overflow-hidden bg-black">
      <div className="absolute inset-0 w-full h-full z-0">
        ...
        {/* 右侧图片展示 */}
        <div className="absolute right-[5%] top-1/2 -translate-y-1/2 w-[600px] hidden lg:flex items-center justify-center">
          <motion.div
            initial={{ opacity: 0, x: 50 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 1.5, ease: [0.16, 1, 0.3, 1] }}
            className="relative w-full rounded-2xl overflow-hidden shadow-[0_0_50px_rgba(0,102,255,0.2)] border border-white/10"
          >
            <img 
              src="/uploaded_image_9.png" 
              alt="AethraGrid Dashboard" 
              className="w-full h-auto object-cover"
              referrerPolicy="no-referrer"
            />
            {/* 叠加一层非常微弱的渐变，让图片更好地融入暗色背景 */}
            <div className="absolute inset-0 bg-gradient-to-tr from-[#0066FF]/10 to-transparent mix-blend-overlay pointer-events-none"></div>
          </motion.div>
        </div>
      </div>
